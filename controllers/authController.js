...
const creaeteSendToken = function (user, statusCode, res) {
...
exports.signUp = catchAsync(async function (req, res, next) {
  const newUser = await User.create({
    ...
    passwordResetExpired: req.body.passwordResetExpired,
    guides: req.body.guides,
  });
  creaeteSendToken(newUser, 201, res);
});
...
exports.login = catchAsync(async function (req, res, next) {
  ...
  if (!email || !password) {
    return next(
      new AppError('Please provide an active email and password', 400)
    );
  }
...
exports.protect = catchAsync(async function (req, res, next) {
  ...
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError(
        'User recenty changed thier password. Please login again!',
        401
      )
    );
  }
